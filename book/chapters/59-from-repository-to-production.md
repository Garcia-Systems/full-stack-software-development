...
At **build time**, the exact direct npm versions declared by this edition compile TypeScript/React into `public/build`, Composer installs PHP packages and optimizes autoloading, and Docker creates `relaydesk:$APP_VERSION`. The version is baked into the image and exposed by liveness. No database, credentials, migrations, or traffic belong to build time. **This edition does not yet commit npm or Composer lockfiles.** Direct constraints are not a substitute for locking transitive dependencies, so two fresh builds can resolve different dependency graphs. CI deliberately uses install/update resolution rather than pretending otherwise. Generate and review both lockfiles in a network-enabled maintenance environment, change builds to consume them strictly, and rerun the complete clean-environment evidence before release preparation.
